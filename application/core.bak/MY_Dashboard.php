...
class MY_Dashboard extends CI_Controller {
    ...
    function Monthly_Requests($year) {
      ...
      function Monthly_Requests_Assignment($year) {
        // echo $query =  $this->db->where('YEAR(designation_date)',date('Y'))->group_by('MONTH(designation_date)')->get('request')->num_rows();
        $query2 = $this->db->query("SELECT MONTHNAME(date_time_tracker) as month, YEAR(date_time_tracker) as year,DATE_FORMAT(date_time_tracker, '%m') as m,
                                    COUNT(id) as 'total'
                                    FROM assigned_samples
                                    WHERE DATE_FORMAT(date_time_tracker, '%Y') = '$year' 
                                    GROUP BY MONTHNAME(date_time_tracker)
                                    ORDER BY MONTH(date_time_tracker) ASC");
       ...
       function Monthly_Requests_Urgent($year) {
        ...
        $query2 = $this->db->query(" SELECT COUNT( DISTINCT si.labref) as total, MONTHNAME(si.date_issued) as month, YEAR(si.date_issued) as year,DATE_FORMAT(si.date_issued, '%m') as m FROM sample_details si WHERE YEAR(si.date_issued) = '$year' GROUP BY MONTHNAME(si.date_issued) ORDER BY MONTH(si.date_issued) ASC  ");
